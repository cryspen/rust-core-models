...
def md_section(label: str, rep: dict) -> str:
    ...
    if out_scope:
        ...
        L += ["", "<details><summary>Out-of-scope modules "
              f"({len(out_scope)} modules, {oc}/{ot} items — platform/runtime "
              "surface not targeted)</summary>", "", names, "</details>"]
    return "\n".join(L) + "\n"
...
def render_markdown(sections: list[tuple[str, dict]], toolchain: str) -> str:
    head = [
        ...
        "hand**. CI fails if this file is out of date; regenerate with "
        "`make coverage`.",
        "",
        f"Denominator: rustdoc JSON of the real `core`/`alloc` "
        f"(`{toolchain}`). Numerator: rustdoc JSON of the model crates. See "
        "`tools/core-coverage/README.md` for what is counted and how modules "
        "are scoped.",
        "",
    ]
